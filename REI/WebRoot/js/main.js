var mainPlatform = {
	...
	bindEvent: function(){
        ...
        //用户信息跳转
        $(document).on('click', '.pf-userinfo', function() {
            $('#pf-page').find('iframe').eq(0).attr('src', 'user!userinfo.action')
        });
        
        //用户信息修改跳转
        $(document).on('click', '.pf-changeuserinf', function() {
            $('#pf-page').find('iframe').eq(0).attr('src', 'user-apply!changeinfo.action')
        });
        
        //修改密码跳转
        $(document).on('click', '.pf-modify-pwd', function() {
            $('#pf-page').find('iframe').eq(0).attr('src', 'user!changepwd.action')
        });
